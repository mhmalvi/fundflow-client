  ...
  return (
    ...
    <div className="w-full max-w-[1280px] mx-auto h-auto lg:h-screen lg:flex bg-admin-color overflow-hidden">
      ...
      <Slide triggerOnce className={`lg:hidden w-full flex flex-wrap justify-around px-4 mx-auto`}>
        <div onClick={()=>{handleToogle(0)}} className={`${toogleTab===0 ? "text-white bg-admin-secondary-color duration-300":"text-black bg-white duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"?"Project":"প্রাজেক্ট"}</div>
        <div onClick={()=>{handleToogle(1)}} className={`${toogleTab===1 ? "text-white bg-admin-secondary-color duration-300":"text-black bg-white duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"?"Notifications":"প্রজ্ঞাপন"}</div>
        <div onClick={()=>{handleToogle(2)}} className={`${toogleTab===2 ? "text-white bg-admin-secondary-color duration-300":"text-black bg-white duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"?"Chat":"চ্যাট"}</div>
        <div onClick={()=>{handleToogle(3)}} className={`${toogleTab===3 ? "text-white bg-admin-secondary-color duration-300":"text-black bg-white duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"?"Member":"সদস্য"}</div>
        <div onClick={()=>{handleToogle(4)}} className={`${toogleTab===4 ? "text-white bg-admin-secondary-color duration-300":"text-black bg-white duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"?"Accounts":"অ্যাকাউন্টিং"}</div>
        <div onClick={()=>{handleToogle(5)}} className={`${toogleTab===5 ? "text-white bg-admin-secondary-color duration-300":"text-black bg-white duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"?"Settings":"সেটিং"}</div>
        <Link href={'./'} className='w-full'>
          <div className={`${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} bg-white rounded-full shadow-md px-5 py-2 my-1 cursor-pointer flex justify-center items-center`}><Image src={Logout} alt='logout' className='w-6 mr-2'/>{lang==="eng"? "Logout":"লগআউট"}</div>
        </Link>
      </Slide>
      </div>
