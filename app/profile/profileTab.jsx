  ...
  return (
    <div className="w-full h-screen lg:flex overflow-x-hidden">
      ...
      <Slide triggerOnce className={`${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[500px]"} lg:hidden w-full flex flex-wrap justify-around px-4 mx-auto`}>
        <div onClick={()=>{handleToogle(0)}} className={`${toogleTab===0 ? "text-white bg-gray-800 duration-300":"text-black bg-white border duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"? "Notifications":"প্রজ্ঞাপন"}</div>
        <div onClick={()=>{handleToogle(1)}} className={`${toogleTab===1 ? "text-white bg-gray-800 duration-300":"text-black bg-white border duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"? "Investments List":"বিনিয়োগ তালিকা"}</div>
        <div onClick={()=>{handleToogle(2)}} className={`${toogleTab===2 ? "text-white bg-gray-800 duration-300":"text-black bg-white border duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"? "Wish Project":"উইশ প্রজেক্ট"}</div>
        <div onClick={()=>{handleToogle(3)}} className={`${toogleTab===3 ? "text-white bg-gray-800 duration-300":"text-black bg-white border duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"? "Recommended":"প্রস্তাবিত"}</div>
        <div onClick={()=>{handleToogle(4)}} className={`${toogleTab===4 ? "text-white bg-gray-800 duration-300":"text-black bg-white border duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"? "Profile":"প্রোফাইল"}</div>
        <div onClick={()=>{handleToogle(5)}} className={`${toogleTab===5 ? "text-white bg-gray-800 duration-300":"text-black bg-white border duration-300"} ${showTabs? "translate-y-0":"absolute -left-56 -translate-x-[1000px]"} w-full text-center rounded-full shadow-md px-5 py-2 my-1 cursor-pointer`}>{lang==="eng"? "Account Settings":"অ্যাকাউন্ট সেটিংস"}</div>
